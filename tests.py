...
class Tester:
    ...
    def plot_result(wins):
        ...
        print("Wins A : %d (%0.1f%%)" %
              (sum_wins[0], (100. * sum_wins[0] / len_wins)))
        print("Wins B : %d (%0.1f%%)" %
              (sum_wins[1], (100. * sum_wins[1] / len_wins)))
        print("Draws  : %d (%0.1f%%)" %
              (sum_wins[2], (100. * sum_wins[2] / len_wins)))
...
class SoccerTester(Tester):
    ...
    def board_to_state(self):
        ...
        state = (state_a * (game.width * game.height - 1) + state_b) + \
            (game.width * game.height) * \
            (game.width * game.height - 1) * game.ball_owner
        return state

    def state_to_board(self, state):
        game = self.game
        ball_owner = state // ((game.width * game.height)
                               * (game.width * game.height - 1))
        state = state % ((game.width * game.height) *
                         (game.width * game.height - 1))
...
def testGame(playerA, playerB, gameTester, iterations):
    ...
    for i in np.arange(iterations):
        ...
        while not gameTester.game.terminal:
            ...
            result, player_ind, terminal = gameTester.game.step(
                action_a, action_b)
            reward = gameTester.result_to_reward(result, action_a, action_b)
            new_state = gameTester.board_to_state()
            if terminal is True:
                if player_ind == 0:
                    playerA.bellman_update(
                        state, new_state, [action_a, action_b], reward)
                    playerB.bellman_update(
                        state, new_state, [action_b, action_a], -reward)
                elif player_ind == 1:
                    playerA.bellman_update(
                        state, new_state, [action_a, action_b], -reward)
                    playerB.bellman_update(
...
def testSoccerPerformance():
    ...
    # INSTANTIATE GAME
    game = Soccer(height=board_height, width=board_width,
    ...
    print('\nApproximate percentage of correct actions : %0.1f%%' %
          (100 * prod / np.sum(v)))
